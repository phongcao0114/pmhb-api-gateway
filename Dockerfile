...
LABEL vendor="kbtg" project="pmhb-api-gateway"
...
COPY . .
RUN ls -l
RUN go build -o pmhb-api-gateway.bin .
RUN chmod +x pmhb-api-gateway.bin
...
EXPOSE 10000
CMD ["./pmhb-api-gateway.bin", "-port", "10000", "-config", "/go/src/app/configs/"]